...
export const evaluateCardFrame = ({
  ...
  const edges: ICardEdges = {
    top: measureHorizontalEdgeScore({ luma, width, height, bandStartRatio: nearStart, bandEndRatio: nearEnd, step }),
    right: measureVerticalEdgeScore({ luma, width, height, bandStartRatio: farStart, bandEndRatio: farEnd, step }),
    bottom: measureHorizontalEdgeScore({ luma, width, height, bandStartRatio: farStart, bandEndRatio: farEnd, step }),
    left: measureVerticalEdgeScore({ luma, width, height, bandStartRatio: nearStart, bandEndRatio: nearEnd, step }),
  };

  const edgeScores = {
    top: edges.top.score, right: edges.right.score,
    bottom: edges.bottom.score, left: edges.left.score,
  };
  const cornerScores = measureAllCornersScores({ luma, width, height, edges, step });
  const detectedWidth = Math.max(0, edges.right.position - edges.left.position) * width;
  const detectedHeight = Math.max(0, edges.bottom.position - edges.top.position) * height;
  const detectedAspect = detectedHeight ? detectedWidth / detectedHeight : 0;
  const expectedAspect = width / height;
  const aspectError = detectedAspect ? Math.abs(Math.log(detectedAspect / expectedAspect)) : Number.POSITIVE_INFINITY;
  const interiorBackgroundContrast = calculateInteriorBackgroundContrast({ luma, width, height, edges, step });
  ...
  const averageCornerScore = (cornerScores.topLeft + cornerScores.topRight + cornerScores.bottomRight + cornerScores.bottomLeft) / 4;
  const minimumEdgeScore = Math.min(edgeScores.top, edgeScores.right, edgeScores.bottom, edgeScores.left);
  const minimumCornerScore = Math.min(cornerScores.topLeft, cornerScores.topRight, cornerScores.bottomRight, cornerScores.bottomLeft);
  const spanCoverage = Math.min(detectedWidth / (width * guide.span), detectedHeight / (height * guide.span));
  ...
  const presenceConfidence = clamp01(
    averageEdgeScore * 0.40 + cornerScores.topLeft * 0.05 + cornerScores.topRight * 0.05 +
      aspectScore * 0.15 + contrastScore * 0.08 + clamp01((spanCoverage - 0.20) / 0.35) * 0.07 + skewScore * 0.10,
  );

  const captureConfidence = clamp01(
    averageEdgeScore * 0.40 + averageCornerScore * 0.15 + contrastScore * 0.12 +
      aspectScore * 0.12 + coverageScore * 0.09 + skewScore * 0.12,
  );
  ...
  return {
    edgeScores, cornerScores, interiorBackgroundContrast, aspectScore, coverageScore,
    presenceConfidence, captureConfidence,
    meetsMinimumGeometry: passesMinGeometry && isInsideGuide(GEOMETRY_RULES.outerTolerance),
    meetsRelaxedGeometry: passesMinGeometry && isInsideGuide(GEOMETRY_RULES.outerTolerance + 0.015),
    skewScore, parallelismScore, minimumEdgeScore, minimumCornerScore, averageEdgeScore, averageCornerScore,
  };
};
...
export const processScannerFrame = ({
  pixels, analysisWidth, analysisHeight, now, previousLuma, currentLuma: inputCurrentLuma,
  readiness, readinessConfig, hasDetectedCard: prevHasDetectedCard,
  isCaptureAligned: prevIsCaptureAligned, thresholds: customThresholds,
  ...
  const { mean, variance, motion, edgeDensity } = analyzeFramePixels({
    pixels, currentLuma, previousLuma, width: analysisWidth, height: analysisHeight, step,
  });
  ...
  const hasPresenceDetails = mean > 24 && mean < 235 && variance > 120 && edgeDensity > 0.003;

  const evalResult = evaluateCardFrame({ luma: currentLuma, width: analysisWidth, height: analysisHeight, step });

  const presenceConfidenceThreshold = prevHasDetectedCard ? thresholds.presenceConfidenceExit : thresholds.presenceConfidenceEnter;
  const hasPresenceCard = hasPresenceDetails && evalResult.presenceConfidence >= presenceConfidenceThreshold;

  const alignmentConfidenceThreshold = prevIsCaptureAligned ? thresholds.alignmentConfidenceExit : thresholds.alignmentConfidenceEnter;
  const isCaptureAligned = hasCardDetails && evalResult.meetsMinimumGeometry && evalResult.captureConfidence >= alignmentConfidenceThreshold;

  const hasDetectedCard = hasPresenceCard || isCaptureAligned;
  const isPreviouslyReady = readiness.isReady;
  const motionThreshold = isPreviouslyReady ? thresholds.motionExitThreshold : thresholds.motionEnterThreshold;

  const isMotionStable = previousLuma !== null && motion < motionThreshold;

  const readinessResult = updateReadinessState({
    state: readiness, isCandidate: isCaptureAligned && isMotionStable, now, config: readinessConfig,
  });

  const isCaptureReady = readinessResult.isReady;
  const scannerStatus: IScannerStatus = isCaptureReady
    ? "stable" : isCaptureAligned ? "aligning" : hasDetectedCard ? "detected" : "searching";

  const distanceHint: IDistanceHint = hasDetectedCard
    ? evalResult.coverageScore < 0.85 ? "too-far" : evalResult.coverageScore > 1.05 ? "too-close" : "fit" : null;

  const debugMetrics: IDebugMetrics = {
    mean, variance, motion: previousLuma ? motion : -1, edgeDensity,
    hasUsableLight, hasCardDetails, hasPresenceDetails,
    presenceConfidence: evalResult.presenceConfidence, alignmentConfidence: evalResult.captureConfidence,
    coverageScore: evalResult.coverageScore, aspectScore: evalResult.aspectScore,
    skewScore: evalResult.skewScore, parallelismScore: evalResult.parallelismScore,
    minEdgeScore: evalResult.minimumEdgeScore, minCornerScore: evalResult.minimumCornerScore,
    avgEdgeScore: evalResult.averageEdgeScore, avgCornerScore: evalResult.averageCornerScore,
    interiorBgContrast: evalResult.interiorBackgroundContrast, meetsMinimumGeometry: evalResult.meetsMinimumGeometry,
  };

  return {
    previousLuma: nextPreviousLuma, currentLuma: nextCurrentLuma,
    readiness: readinessResult.nextState, scannerStatus, hasDetectedCard,
    isCaptureAligned, isCaptureReady, distanceHint, debugMetrics,
  };
};
